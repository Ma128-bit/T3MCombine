   ...
   for(int cr = 0; cr < 3; cr++) {
      ...
      for (unsigned int category=0; category < NCAT; category++){
          ...
          if(cat_name.rfind(cat_reso[cr], 0) == 0){ //A1,A2,A3
              ...
              sprintf(line, "(1+0.0015*%s)*%.5f", "UncMean", sig_m0->getVal()*0.9991); // mass mean value
              ...
              sprintf(line2, "(1+0.08*%s)*%.5f", "UncSigma", sigma[category]->getVal()); // mass mean value
...
MakePlots(RooWorkspace* w, const Int_t NCAT, std::vector<string> cat_names, bool dp, bool blind, string configs, TString type, TString Run){
   ...
   for(unsigned int category=0; category< NCAT; category++){
      ...
      if(Run=="2022") lumiText+=", 34.2 fb^{-1} (13.6 TeV)";
...
AddData(TString file, TString era, RooWorkspace* w, const Int_t NCAT, std::vector<string> branch_names, std::vector<string> cat_names, std::vector<string> bdt_val){
   ...
   TString omega_cut = "abs("+dimu1_name+"-0.782)>0.001 && abs("+dimu2_name+"-0.782)>0.001 && abs("+dimu1_name+"-1.019)>0.001 && abs("+dimu2_name+"-1.019)>0.001";
   //TString omega_cut = "abs("+dimu1_name+"-0.782)>0.00";
   ...
   RooRealVar dimu1(dimu1_name, dimu1_name, 0.2, 1.9); //dimu_OS1
   RooRealVar dimu2(dimu2_name, dimu2_name, 0.2, 1.9); //dimu_OS2
...
void MakeDataCard(RooWorkspace* w, const Int_t NCAT, const char* fileBaseName, const char* fileBkgName, string configFile,  std::vector<string> cat_names, TString type, TString Run, bool blind, bool dp){
   ...
   for (int c = 0; c < NCAT; ++c) {
      ...
      if (Run.Contains("2022")){
         if (cat_name.Contains("1"))  outFile << "UncBDTCut         lnN  1.10	   - " << endl; //unc. on BDT selection
         if (cat_name.Contains("2"))  outFile << "UncBDTCut         lnN  1.10	   - " << endl; //unc. on BDT selection
         if (cat_name.Contains("3"))  outFile << "UncBDTCut         lnN  1.10	   - " << endl; //unc. on BDT selection
      }
      ...
      if (Run.Contains("2018")){ 
         ...
         outFile << "UncL1_18       lnN  1.0016     - " << endl; // events triggered by TripleMu
      ...
      if (Run.Contains("2022")){
         outFile << "DsNorm_22      lnN  1.093	   - " << endl; // normalisation factor computed on control channel
         outFile << "fUnc_22        lnN  1.02      - " << endl; // B/D ratio
         outFile << "UncHLT_22      lnN  1.05      - " << endl; // uncertanty on 2018 HLT
         outFile << "UncL1_22       lnN  1.04	   - " << endl; // events triggered by TripleMu
         ...
         if(type=="threeGlobal"){
             ...
             outFile << "UncMuonEff_22_3glb  lnN  1.015     - " << endl; //GlobalMu ID tag&probe
      ...
      if (Run.Contains("2017")){
         ...
         outFile << "fUnc_17        lnN  1.02      - " <<endl;  //B/D ratio, lascia uguale
